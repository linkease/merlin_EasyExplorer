...
function get_status() {
    $j.ajax({
        url: 'apply.cgi?current_page=Module_easyexplorer.asp&next_page=Module_easyexplorer.asp&group_id=&modified=0&action_mode=+Refresh+&action_script=&action_wait=&first_time=&preferred_lang=CN&SystemCmd=easyexplorer_status.sh',
        dataType: 'html',
        error: function(xhr) {
            alert("error");
        },
...
function updateOptions(){
    ...
    document.form.SystemCmd.value = "easyexplorer_config.sh";
...
function menu_hook(title, tab) {
    ...
    if(enable_ss == "1" && enable_soft == "1"){
        tabtitle[tabtitle.length -2] = new Array("", "EasyExplorer文件同步");
        tablink[tablink.length -2] = new Array("", "Module_easyexplorer.asp");
    }else{
...
function version_show(){
    $j.ajax({
        url: 'https://koolshare.ngrok.wang/easyexplorer/config.json.js',
        type: 'GET',
        dataType: 'jsonp',
